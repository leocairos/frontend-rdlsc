...
export type ListItemLinkProps = NavLinkProps;

export interface IListItemComponentProps extends HTMLAttributes<HTMLElement> {
  link?: string | null;
  children?: any;
...
export const ListItemComponent: React.ExoticComponent<IListItemComponentProps> = forwardRef(
  (props: IListItemComponentProps, ref: React.Ref<HTMLDivElement>) => {
    // Omit isCollapsed
    ...
    return (
      <div
        ref={ref}
        className={clsx(isCollapsed && classes.navItemCollapsedWrapper)}
      >
        {component}
      </div>
    );
...
const SidebarNavItem: React.FC<ISidebarNavItemProps> = (
  ...
  function getItemsAll(
    itemsSide: ISidebarNavItemProps[],
  ): ISidebarNavItemProps[] {
    return itemsSide.reduce(
      (allItems: ISidebarNavItemProps[], item: ISidebarNavItemProps) => {
